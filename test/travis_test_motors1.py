...
class MotorTest(unittest.TestCase):
    def file_check(self, dev, value, message):
        with open("/dev/" + dev, "r") as f:
            self.asserEqual(f.readline(), str(value) + "\n", message)

    def test_node_exit(self):
        nodes = rosnode.get_node_names()
        self.assertIn('/motors', nodes, "node does node exitst")
    ...
    def test_put_freq(self):
        ...
        self.file_check("rtmotor_raw_l0", m.left_hz, "Wrong left vale from motor_raw")
        self.file_check("rtmotor_raw_r0", m.right_hz, "Wrong right vale from motor_raw")
    ...
    def test_put_cmd_vel(self):
        ...
        self.file_check("rtmotor_raw_l0", 200, "Wrong left vale from cmd_vel")
        self.file_check("rtmotor_raw_r0", 600, "Wrong right vale from cmd_vel")
